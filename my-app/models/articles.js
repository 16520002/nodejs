    ...
    if ( typeof cids === 'object') {
        for (let index = 0; index < cids.length; index++) {
          ...
          await ItemServer.find({_id:cids[index]}).then(rs=>{
            fileHelpers.removeFile(`public/uploads/${collectionName}/`,rs[0].avatar)
          })
...
async function changePosition(id='',position=''){
  ...
  if ( typeof id === 'object') {
      await ItemServer.updateMany({_id:{$in:id}}, data).then(rs=>{
        ...
        await ItemServer.updateOne({_id: id},data)
        result.notify = notifyConfig.CHANGE_POSITION_SUCCESS
    }
      return result
}
...
async function manyItems(item){
    ...
    if(item.id !== '' && item !== undefined){
      if(item.avatar === undefined) {
        ...
        fileHelpers.removeFile(`public/uploads/${collectionName}/`,item.image_old)
      }
        //!error.isEmpty()
        ...
        // item.category = {
        //     id: JSON.parse(item.category_id)._id,
        //     name: JSON.parse(item.category_id).name
        // }
      ...
      result.notify = notifyConfig.ADD_ITEM_SUCCESS
        await new ItemServer(item).save().then(async(room)=>{
        
          let articleArr = await CategoryServer.findById({_id:room.categories})
          articleArr.categories.push(room)
          await CategoryServer(articleArr).save()
        })//.then(re => req.flash('success' ,notifyConfig.ADD_ITEM_SUCCESS ,linkIndex))
        return result
      }
        return result
}
...
async function listItem(store){
    ...
    if(store.categoryId!=='') {objwhere={categories:store.categoryId}}
    await ItemServer.count(store.objwhere)
      ...
      if(store.categoryId !=='' && store.categoryId !== 'allcategory' ) {store.objwhere={'category.id': store.categoryId,...store.objwhere}}
    if(store.category){
      if (store.category.name||store.category.id) {
        store.category.name='',
        store.category.id=''
      }
    }

    //let log = await CategoryServer.findById({_id:store.categories})

    return await ItemServer
      .find(store.objwhere)
      //.select('fullName userName email category status ordering deleted editorData create modify createdAt updatedAt _id')
      .sort(store.sort)
...
async function listItemFrontend(params,options=null){
  if(options.task === 'special-item'){
    return ArticleServer
    .find({status:'active',position:'toppost'})
    .sort({ordering:'asc'})
    .limit(3)
  }
}
